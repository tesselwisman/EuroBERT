...
This section defines your model's architecture. If you provide a huggingface_id, other parameters are locked. Otherwise, for custom models, unspecified parameters will use defaults based on the model size.
...
| model_name | str | bert | Model type (bert or eurobert). |
| model_size | str | 3b | Model size (e.g., 210m, 3b). |
...
Pro Tip: Want to create a custom model? Check out the example in optimus/trainer/model/encoder/eurobert.py within this repository!
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| data_mix_path | str | optimus/trainer/configuration/data_mix | Path to data mix config (relative to project root). |
...
| optimizer | str | AdamW | Default optimizer. |
...
| lr_scheduler | str | WarmupStableDecayLR | LR scheduler. |
...
| end_start | float | 0 | Percentage of iterations for decreasing the learning rate. |
